function (element, input) {
    ...
    function findAncestorByTagName(target, tagName) {
        var current = target;
        var expectedTagName = String(tagName || "")
            .toUpperCase();

        while (current) {
            if (
                current.tagName &&
                String(current.tagName).toUpperCase() ===
                    expectedTagName
            ) {
                return current;
            }
    ...
    function getTableRowText(target) {
        try {
    ...
    function getTableCellText(target) {
        try {
    ...
    function getTableColumnIndex(target) {
        try {
            var cell = getTableCell(target);
        ...
        try {
            ...
            for (var rowIndex = 0; rowIndex < rows.length; rowIndex++) {
                ...
                if (
                    ...
                    var headerCell = row.cells[columnIndex];
                    var headerTagName = headerCell.tagName
                        ? String(headerCell.tagName).toUpperCase()
                        : "";

                    if (headerTagName === "TH") {
                        var headerText = getVisibleText(headerCell);

                        if (
                            headerText &&
                            headerParts.indexOf(headerText) < 0
                        ) {
                            headerParts.push(headerText);
                        }
                    } else if (!fallbackHeader) {
                        var fallbackText = getVisibleText(headerCell);

                        if (
                            fallbackText &&
                            String(fallbackText).toUpperCase() !== "PDF"
                        ) {
                            fallbackHeader = fallbackText;
                        }
                    }
                }
            }
    ...
    function getTableContext(target) {
        try {
            ...
            for (var index = 0; index < row.cells.length; index++) {
                var cellText = truncate(
                    getVisibleText(row.cells[index]),
                    MAX_HINT_LENGTH
                );
                ...
                for (var rowIndex = 0; rowIndex < rows.length; rowIndex++) {
                    ...
                    if (rows[rowIndex].cells && rows[rowIndex].cells.length > index) {
                        var headerCell = rows[rowIndex].cells[index];
                        var headerTag = headerCell.tagName
                            ? String(headerCell.tagName).toUpperCase()
                            : "";

                        if (headerTag === "TH") {
                            var visibleHeader = getVisibleText(headerCell);

                            if (visibleHeader) {
                                headerText = visibleHeader;
                            }
                        }
                    }
                }
